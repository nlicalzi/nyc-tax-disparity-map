...
priority tiers (see 09_fetch_geometry.py for why two extra datasets are
needed beyond PLUTO):
...
Usage:
    .venv/bin/python pipeline/10_join_geometry.py
...
def main():
    ...
    # time (pipeline/11_build_tileset.py).
    ...
    con.execute(f"""
        COPY (
            SELECT
                ...
                -- v2 roadmap (PLAN.md): owner-entity + stacked-benefit fields,
                -- carried through geometry join the same as everything else.
                llc_unit_count, llc_unit_pct, sample_owner,
                has_421a_exemption, exemption_421a_amt,
                has_j51_exemption, j51_exemption_amt,
                has_coop_condo_abatement, coop_condo_abatement_amt,
                has_j51_abatement, j51_abatement_amt,
                -- v2 roadmap item 1: dollar gap vs. the class-1 trend line
                -- (07_compute_dollar_gap.py) -- see that script's module
                -- docstring for why this is often 0, including for Griffin.
                building_dollar_gap_vs_class1_trend
            FROM buildings_geo
