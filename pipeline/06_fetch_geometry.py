...
Real polygons live in two datasets from DOF's Digital Tax Map collection:
  ...
  - Digital Tax Map: Condominiums (p8u6-a6it): crosswalk from a condo's
    ...
    time -- see 07_join_geometry.py.

The remaining ~0.13% of buildings resolve to neither and fall back to
PLUTO's point centroid in 07_join_geometry.py.

Usage:
    .venv/bin/python pipeline/06_fetch_geometry.py --sample   # Manhattan only
    .venv/bin/python pipeline/06_fetch_geometry.py            # full citywide
"""
import argparse
import json
import os
